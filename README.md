...
#### Extra explanation:
A regular linked list uses one pointer on each node to point to the next node, but in a doubly linked list each node has two pointers, one pointing to the next node and the other pointing to the previous node
